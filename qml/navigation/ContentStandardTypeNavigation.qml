...
Page {
    ...
    property string name: "ContentStandardTypeNavPage"
    ...
    StackView {
        id: navPane
        anchors.fill: parent
        property string name: "ContentStandardTypeNavPane"
        focus: true

        initialItem: ContentStandardTypeListPage {
            id: initialItem
        }

        Loader {
            id: contentStandardTypeKeyListPageLoader
            property int contentStandardTypeId: -1
            active: false
            visible: false
            source: "../pages/ContentStandardTypeKeyListPage.qml"
            onLoaded: {
                item.contentStandardTypeId = contentStandardTypeId
                navPane.push(item)
                item.init()
            }
        }
        ...
        function pushContentStandardTypeKeys(contentStandardTypeId) {
            contentStandardTypeKeyListPageLoader.contentStandardTypeId = contentStandardTypeId
            contentStandardTypeKeyListPageLoader.active = true
        }
        ...
        function popOnePage() {
            var page = pop()
            if(page.name == "contentStandardTypeKeyListPage") {
                contentStandardTypeKeyListPageLoader.active = false
                return
            }
    ...
    FloatingActionButton {
        visible: navPane.depth == 2 && !dataManager.settingsData().classicStackNavigation
        property string imageName: "/folder_fav.png"
        z: 1
        anchors.margins: 20
        anchors.right: parent.right
        anchors.bottom: parent.bottom
    ...
    function init() {
        console.log("INIT ContentStandardTypeNavPane")
        initialItem.init()
    }
    function cleanup() {
        console.log("Cleanup ContentStandardTypeNavPane")
    }
